...
```{r remove_blank_taxa, echo=T, eval=T, include=T}

# RUN DECONTAM: not done yet
psdata_blank
...
# mock taxa needed to isolate blank NC contaminants
mock_taxa = c("Brevibacillus","Weizmannia","Lysinibacillus","Bacillus","Sphingobium","Serratia","Streptomyces","Burkholderia","Burkholderia-Caballeronia-Paraburkholderia","Novosphingobium","Genus of Enterobacteriaceae", "Genus of Yersiniaceae")
# create single string with pipes between mock taxa for subsetting
mock_taxa_or = "Brevibacillus|Weizmannia|Lysinibacillus|Bacillus|Sphingobium|Serratia|Streptomyces|Burkholderia|Burkholderia-Caballeronia-Paraburkholderia|Novosphingobium|Genus of Enterobacteriaceae| Genus of Yersiniaceae"


# remove mock ASVs from blank
contamination_blank = 
psdata_blank %>% 
  subset_taxa(!Genus %in% mock_taxa_or) %>% 
  psmelt() %>% 
  group_by(Sample, Genus) %>% 
  summarise(sum_abund = sum(Abundance)) %>% 
  group_by(Genus) %>% 
  summarise(mean_abund = mean(sum_abund), 
            n = sum(sum_abund != 0),
            se = mean_abund / sqrt(n))

# view absolute abundance of mock Genera in samples ~ observed mock abundance
contamination_blank %>% 
  inner_join(.,
             psdata_mock %>% 
               subset_taxa(Genus %in% mock_taxa) %>% 
               psmelt() %>% 
               select(Sample, Genus, Abundance) %>% 
               group_by(Sample, Genus) %>% 
               summarise(mock_abund = sum(Abundance), .groups = "drop"),
             by = "Genus") %>% 
  ggplot(aes(x = mock_abund, y=mean_abund)) +
  # geom_errorbar(aes(ymin = mean_abund - se,
  #                   ymax = mean_abund + se, 
  #                   color = Genus)) +
  geom_point(aes(fill = Genus),
             shape = 21, color = "grey70", size = 2,) +
  labs(x = "Genus abundance in Mock community",
       y = "Mock Genus abundance in Blank (\u00B1SE)") +
  # geom_text(aes(label = "n samples =", x = -3000, y = -20), nudge_x = 15000, color = "grey50",  size = 3, show.legend = F) +
  # geom_text(aes(label = paste0(n), color = Genus, x=mock_abund+(max(mock_abund)/30)), size = 3, show.legend = F) +
  theme_light() +
  theme(panel.grid = element_blank())


# recreate psdata with samples only and filtering mock ASVs
psdata = psdata_samples %>% subset_taxa(!grepl(mock_taxa_or, Genus))
# recreate psdata with samples only
psdata = psdata_samples

```

```{r remove_mock, echo=T, eval=T, include=T}
...
contamination_mock %>% 
  ...
  geom_point(aes(fill = Genus),
             shape = 21, color = "grey70", size = 2,) +
  labs(x = "Genus abundance in Mock community",
       y = "Mock Genus abundance in samples (\u00B1SE)") +
  geom_text(aes(label = "n samples =", x = -3000, y = -20), nudge_x = 15000, color = "grey50",  size = 3, show.legend = F) +
  geom_text(aes(label = paste0(n), color = Genus, x=mock_abund+(max(mock_abund)/30)), size = 3, show.legend = F) +
  theme_light() +
  theme(panel.grid = element_blank())
...
ggsave2(plot = heatmap_genus_3pct + 
                ggtitle(label = "Genus heatmap - pH and dual vs. single set-up"), 
...
heatmap_data_long %>% 
  ...
  group_by(Set_up, tax_label, pH, pH_factor) %>% 
  pivot_wider(names_from = "Timeline", values_from = "sum_rel_abund") %>% 
...
heatmap_data_change = 
heatmap_change_long %>% 
select(Set_up, tax_label, change) %>% 
  pivot_wider(names_from = tax_label, 
              values_from = change) 

# make heatmap
m_change <- as.matrix(heatmap_data_change[, 4:ncol(heatmap_data_change)])
clust_change <- hclust(dist(t(m_change)))

# heatmap change
heatmap_change = 
heatmap_change_long %>% 
ggplot(aes(pH_factor, tax_label)) +
  geom_tile(aes(fill = change), color = NA) +
  labs(x = NULL, y=NULL) +
  scale_y_discrete(limits = colnames(m_change)[rev(clust_change$order)]) +
  ...
  geom_text(
  aes(label = ifelse(abs(change) >1 & abs(change) < 10, round(change, digits = 0), "")),
  color = "grey50", size =2.5) +
  geom_text(
  aes(label = ifelse(change == 0, ".", "")),
  color = "grey80", size = 5, position = position_nudge(y=0.5)) +
  geom_text(
  aes(label = ifelse(abs(change) > 10, round(change, digits = 0), "")),
  color = "grey90", size = 2.5) +
  facet_nested(~Set_up + pH_factor,
               labeller = labeller(Set_up = setup_label,
                                   pH_factor = pH_label),
               scales = "free", 
               nest_line = element_line(lineend = "round"), 
...
print(heatmap_change)

ggsave2(plot = heatmap_change + 
                ggtitle(label = "Genus heatmap change - pH and dual vs. single set-up"), 
        filename = glue("figures/{proj}_heatmap_change_genus.pdf"), width = 9, height = 7)
